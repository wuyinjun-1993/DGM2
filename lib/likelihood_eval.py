...
# 		t2 = time.time()
		...
		log_prob = -torch.sum(0.5*(((data_2d - mu_2d)/obsrv_std)**2 + 2*np.log(obsrv_std) + 2*np.log(np.sqrt(2*np.pi))))
		
# 		t3 = time.time()
# 		
# 		print(torch.norm(log_prob - log_prob0))
# 		
# 		print('time 1:', t2 - t1)
# 		
# 		print('time 2:', t3 - t2)
...
def compute_binary_CE_loss(label_predictions, mortality_label):
	...
	if len(idx_not_nan) == 0.:
		...
		ce_loss = torch.Tensor(0.).to(mortality_label.device)
...
def compute_multiclass_CE_loss(label_predictions, true_label, mask):
	...
	ce_loss = torch.stack(res, 0).to(label_predictions.device)
...
def mse(mu, data, indices = None):
	...
	if n_data_points > 0:
		...
		mse = torch.zeros([1]).to(data.device).squeeze()
	return mse



def mae(mu, data, indices = None):
	n_data_points = mu.size()[-1]

	if n_data_points > 0:
		mae = torch.abs(mu - data)
# 		mse = nn.MSELoss()(mu, data)
	else:
		mae = torch.zeros([1]).to(data.device).squeeze()
	return mae
...
def compute_mae(mu, data, mask = None):
	# these cases are for plotting through plot_estim_density
	if (len(mu.size()) == 3):
		# add additional dimension for gp samples
		mu = mu.unsqueeze(0)
