...
#                 self.z_dim = self.input_dim
        ...
        self.use_mask = True
        if self.use_mask:
            self.x_encoder = Encoder_cluster(2*self.z_dim, self.s_dim, self.dropout, self.device, bidir = False, block = self.block, n_layers =self.lstm_layer)
        else:
            self.x_encoder = Encoder_cluster(self.z_dim, self.s_dim, self.dropout, self.device, bidir = False, block = self.block, n_layers =self.lstm_layer)
        
        if self.transfer_prob:
            self.postnet = PostNet_cluster(self.cluster_num, self.s_dim, self.cluster_num, self.dropout, self.sample_times, bidirt = self.x_encoder.bidir)
        else:
            self.postnet = PostNet_cluster(self.z_dim, self.s_dim, self.cluster_num, self.dropout, self.sample_times, bidirt = self.x_encoder.bidir)
...
#         self.x_encoder = torch.nn.LSTM(self.input_dim, self.s_dim, bidirectional = True)
        ...
        if self.transfer_prob:
            if self.block == 'GRU':
                self.trans = torch.nn.GRU(self.cluster_num, self.h_dim, num_layers = self.lstm_layer, dropout = self.dropout, batch_first = True)
            else:
    #             self.trans = Encoder_cluster(self.input_dim, self.h_dim, self.dropout, bidir = False, block = self.block)
                self.trans = torch.nn.LSTM(self.cluster_num, self.h_dim, num_layers = self.lstm_layer, dropout = self.dropout, batch_first = True)
        
        else:
            if self.block == 'GRU':
                self.trans = torch.nn.GRU(self.z_dim, self.h_dim, num_layers = self.lstm_layer, dropout = self.dropout, batch_first = True)
            else:
    #             self.trans = Encoder_cluster(self.input_dim, self.h_dim, self.dropout, bidir = False, block = self.block)
                self.trans = torch.nn.LSTM(self.z_dim, self.h_dim, num_layers = self.lstm_layer,dropout = self.dropout, batch_first = True)
...
#                 self.phi_table = torch.zeros([2*self.z_dim, self.cluster_num], dtype = torch.float, device = config['device'])
#             else:
...
#         return x
    ...
    def generate_z(self, z_category, prior_cluster_probs, t, h_now=None):
        ...
        if self.use_gate:
            curr_gaussian_coeff = (self.gaussian_prior_coeff + F.sigmoid(self.gate_func(h_now[-1])))/2
...
#         return torch.t(all_distances)
    ...
    def sample_x(self, phi_z, z_prob):
...
#         avg_selected_phi_z += std*epsilon 
        
#         print(torch.norm(avg_selected_phi_z - torch.t(phi_z)))
        
#         avg_sampled_x = torch.mean(sampled_x, )
...
#         x = m.sample()
#         if not self.cluster_mask:
        return mean, logvar, mean
#         else:
#             return mean[:,0:self.z_dim], logvar[:,0:self.z_dim], mean[:,0:self.z_dim]
...
#         log_prob = gaussian.log_prob(x[0][0])
    ...
    def compute_rec_loss(self, joint_probs, prob_sums, full_curr_rnn_input, rnn_out, x_t, x_t_mask, h_now = None, curr_z_t_category_infer=None):
...
#                 
#                 if self.cluster_mask:
#                     input_x_t = torch.cat([x_t, x_t_mask], -1)
#                     
#                     input_x_t_mask = torch.cat([x_t_mask, torch.ones_like(x_t_mask)], -1)
#                 
#                 else:
#                     input_x_t = x_t
#                     
#                     input_x_t_mask = x_t_mask    
#                 
#                 x_t_full = input_x_t.view(1, input_x_t.shape[0], input_x_t.shape[1])
#             
#                 x_t_full = x_t_full.repeat(self.cluster_num, 1, 1)
#                 
#                 x_t_mask_full = input_x_t_mask.view(1, input_x_t_mask.shape[0], input_x_t_mask.shape[1])
#                 
#                 x_t_mask_full = x_t_mask_full.repeat(self.cluster_num, 1, 1)
#                 x_t_mask_full = x_t_mask_full.repeat(self.cluster_num, 1, 1)
...
#         print('time 2::', t3 - t2)
        if self.cluster_mask:
#             full_logit_x_t = torch.mm(joint_probs, torch.t(self.phi_table))[:, 0:x_t.shape[-1]]
...
#         torch.sum(full_rec_loss).backward(retain_graph=True)
    ...
    def update_joint_probability2(self, joint_probs, curr_rnn_output, batch_size, t, h_prev, c_prev, prev_z_t_category_infer, shrinked_x_lens, x_t, x_t_mask):
...
#             t2 = time.time()
            ...
            imputed_x, interpolated_x = self.impute.forward2(x, x_mask, torch.tensor(range(T_max), device =self.device, dtype = torch.float))
...
#         else:
#             if not self.lstm_latent:
#                 rnn_out,(last_h_n, last_c_n)= self.x_encoder(self.x_kernel_encoder(input_x), x_lens) # push the observed x's through the rnn;
#             else:
#                 rnn_out,(last_h_n, last_c_n)= self.x_encoder(input_x, x_lens) # push the observed x's through the rnn;
...
#                 kl_states[curr_x_lens > 0,t] = kl
                ...
                '''joint_probs, curr_rnn_output, batch_size, t, h_prev, c_prev, shrinked_x_lens'''
                '''updated_joint_probs, full_kl, h_now, c_now, full_rec_loss, full_logit_x_t'''
                updated_joint_probs, kl, h_now, c_now, z_t_category_infer, z_t_category_gen = self.update_joint_probability2(joint_probs[t-1, curr_x_lens > 0], curr_rnn_out, torch.sum(curr_x_lens > 0), t, h_prev, c_prev,z_t_category_infer, shrinked_x_lens, x_t, x_t_mask)
...
#                 else:
                ...
                full_rec_loss1, full_rec_loss2, full_logit_x_t, curr_full_rec_loss, l2_norm_loss, cluster_loss = self.compute_rec_loss(joint_probs[t, curr_x_lens > 0], prob_sums/torch.sum(x_lens), full_curr_rnn_input, rnn_out[curr_x_lens > 0,t,:], input_x_t, x_t_mask, curr_rnn_out, all_z_t_category_infer[t])
...
#                     z_t_category_gen = z_t_category_gen.view(1, z_t_category_gen.shape[0], z_t_category_gen.shape[1])
            ...
            phi_z, z_representation = self.generate_z(z_t_category_gen, prior_cluster_probs, T_max_train + t, last_h_n)
...
#             print(phi_z.shape)
            ...
            avg_selected_phi_z = self.sample_x(phi_z, z_representation)
...
#                     output, h_now = self.trans(z_t_category_gen_trans, h_now)# p(z_t| z_{t-1})
...
#                     output, (h_now, c_now) = self.trans(z_t_category_gen_trans, (h_now, c_now))# p(z_t| z_{t-1})
